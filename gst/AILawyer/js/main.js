$(document).ready(function () {
  ...
  // 계정 옆 dropdown 화살표 클릭 시 badge text 노출
  $(".dropdown_w").on("mouseenter", function () {
      $('.account_wrap .badge').addClass('hover');

      $('.account_wrap .badge_txt').css({
          visibility: 'visible',
          opacity: '1',
  ...
  $(".dropdown_w").on("mouseleave", function () {
      $('.account_wrap .badge').removeClass('hover');
  ...
  // sel_w에서 이벤트 전파 차단
  $(".sel_w").on("mouseenter", function (event) {
      event.stopPropagation(); // 이벤트 전파 차단
  });

  $(".sel_w").on("mouseleave", function (event) {
      event.stopPropagation(); // 이벤트 전파 차단
  });

  // 내정보 수정 > 분야 선택
  $('.field_area button').click(function () {
      $(this).toggleClass('active');
  });
...
//회원가입 유형 선택 - hover
$(document).ready(function () {
  $('.select_signup button').hover(function () {
      $(this).siblings('.select_option').addClass('hover');
  }, function () {
      $(this).siblings('.select_option').removeClass('hover'); // hover 해제 시 클래스 제거
  });
});