$(document).ready(function () {
    ...
    // -- fade-left 애니메이션
    const fadeLeftItems = $(".fade-left");
    const handleFadeLeftScroll = () => {
        fadeLeftItems.each(function (index) {
            const $this = $(this);
            const elementTop = $this.offset().top;
            const viewportBottom = $(window).scrollTop() + $(window).height();

            if (elementTop < viewportBottom + 100 && !$this.hasClass("visible")) {
                setTimeout(() => $this.addClass("visible"), index * 200); // 200ms 간격
            }
        });
    };

    $(window).on("scroll resize load", handleFadeLeftScroll);
    handleFadeLeftScroll();
